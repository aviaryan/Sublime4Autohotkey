/* 
##########################################################
LQ-CustomBuild - MINIMALIST APPLICATION LAUNCHER
v1.0 
##########################################################
...
SetTimer, endtask, 1500
AddonsDir := Substr(A_scriptdir, 1, Instr(A_scriptdir, "\", false, 0)) "Addons"
ahkpath := Substr(A_scriptdir, 1, Instr(A_scriptdir, "\", false, 0, 2)) "autohotkey.exe"
cl_su4ahk := new talk("Sublime4Autohotkey.ahk")
...
version := 1.0 , color := "0x5a87b4" , textcolor := "Black" , transparency := 216
sizey := 500 , sizex := 350
...
If !(FileExist("q-settings/settings.ini"))
{
	...
	loop, %AddonsDir%\*, 0, 1
		names_list .= A_loopfilename "`n" , paths_list .= A_LoopFileFullPath "`n" , numberofitems := A_index
	names_list := RTrim(names_list, "`n") , paths_list := Rtrim(paths_list, "`n")

	FileAppend,%names_list%,q-settings/names.lq
	FileAppend,%paths_list%,q-settings/paths.lq
	FileAppend,%numberofitems%`n%version%`n!a`n100`n%color%`n%textcolor%`n%transparency%,q-settings/settings.ini		;appending

	Arrange() , names_list := "" , paths_list := ""
}
else
{
	FileDelete,q-settings/paths.lq
	...
	Fileatline("q-settings/settings.ini", numberofitems, 1)
}
...
Gui, Add, Text, x0 y0 w%sizex% h30 +Center gabout, LQ-CustomBuild
...
Gui, 3:Add, Text, x2 y0 w590 h30 +Center gpage, LQ-CustomBuild v%version% : by Avi Aryan
...
Menu, Tray, Tip, LQ-CustomBuild by Avi Aryan
Menu, Tray, Add, LQ-CustomBuild, about
Menu, Tray, Add,
Menu, Tray, Add, Online Help, help
Menu, Tray, Add, Run at Startup, startup
Menu, Tray, Add
Menu, Tray, Add, Quit, quit
Menu, Tray, Default, LQ-CustomBuild

;OTHERS
IfExist,%a_startup%/LQ-CustomBuild.lnk
{
FileDelete,%a_startup%/LQ-CustomBuild.lnk
FileCreateShortcut,%A_ScriptFullPath%,%A_Startup%/LQ-CustomBuild.lnk
...
IfLess,x,%sizex%
	...
	Tooltip, %path%
		...
		Tooltip, LQ-CustomBuild - Click to open Settings
	else if classnn = 12
		ToolTip, Add web link to LQ-CustomBuild
	else
		ToolTip
	}
;Mouseget
...
if (Winactive("LQ-CustomBuild ahk_class AutoHotkeyGUI") AND title == "LQ-CustomBuild")
{
MouseGetPos,,,,classnn
...
;#############################################################
ShowGUI:
IfWinNotExist, LQ-CustomBuild ahk_class AutoHotkeyGUI
{
...
Gui, 1:Show, x%xax% y%yax% w%sizex% h%sizey%, LQ-CustomBuild
WinSet, Region, 0-0 W%sizex% H%sizey% R40-40, LQ-CustomBuild ahk_class AutoHotkeyGUI
Winset, Transparent, %transparency%, LQ-CustomBuild ahk_class AutoHotkeyGUI
WinActivate,LQ-CustomBuild
...
Gui, 3:Show, w595 h310, LQ-CustomBuild by Avi
return
help:
...
update:
URLDownloadToFile,https://dl.dropboxusercontent.com/u/116215806/Products/LQ-CustomBuild/version.txt,q-settings/latest.txt
...
;###################################################################
#IfWinActive, LQ-CustomBuild by Avi
{
!space::
...
EndFunction(){
	...
	SetTitleMatchMode 2
    if !Winexist("Sublime4Autohotkey.ahk - ahk_class AutoHotkey")
		return 1
    DetectHiddenWindows %Prev_DetectHiddenWindows%
    SetTitleMatchMode %Prev_TitleMatchMode%
}

#include, %A_ScriptDir%/local_talk.ahk